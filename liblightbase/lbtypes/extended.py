...
class MetaMultiple(lbtype):
    ...
    def __init__(self, value=None, name=''):
        """ Generate from structure, 
            ...
            parameters.
        """
        for key,atype in list(self._structure.items()):    
...
class MetaArray(list, lbtype):
    ...
    def __xml__(self, doc):
        ...
        for value in values:
            if '_value' in value.__dict__:
                item = doc.createElement('valor')
                item_text = doc.createTextNode(str(value.__dict__.get('_value')))
